...
DEPS="ntp git-core build-essential m4 autoconf libpcre3-dev libssl-dev \
      libncurses5-dev libssh-dev libdb-dev libdb++-dev libboost-all-dev"
aptitude -y install $DEPS

# Install bitcoind
source recipes/bitcoind.sh $(cat attributes/tmp_path) $(cat attributes/bitcoind_git_ref)
...
# Install rebar
source recipes/rebar.sh $(cat attributes/tmp_path) $(cat attributes/rebar_git_ref)

# Install Twitcoin
source recipes/twitcoin.sh $(cat attributes/tmp_path)
